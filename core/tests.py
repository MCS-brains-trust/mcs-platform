...
class TaxPlanningUXTests(TestCase):
    """
    Tests for the Tax Planning finalisation gate error wording and the
    HTMX auto-save endpoint introduced to prevent silent unsaved-changes
    data corruption.
    """

    def _create_admin_user(self):
        return User.objects.create_user(
            username=f"tp_admin_{uuid.uuid4().hex[:8]}",
            password="testpass123",
            role=User.Role.ADMIN,
    ...
    def _create_tax_planning_worksheet(self, distributable_income):
        from core.models import TaxPlanningWorksheet
        client_obj = Client.objects.create(name="TP Test Client")
        entity = Entity.objects.create(
            entity_name="TP Test Trust",
            entity_type="trust",
            client=client_obj,
        )
        fy = FinancialYear.objects.create(
        ...
        worksheet = TaxPlanningWorksheet.objects.create(
            financial_year=fy,
            distributable_income=Decimal(str(distributable_income)),
        )
        return worksheet

    def _add_beneficiary_row(self, worksheet, proposed_distribution):
        from core.models import TaxPlanningBeneficiaryRow
        officer = EntityOfficer.objects.create(
            entity=worksheet.financial_year.entity,
            full_name=f"Beneficiary {uuid.uuid4().hex[:6]}",
            role=EntityOfficer.OfficerRole.BENEFICIARY,
        )
        return TaxPlanningBeneficiaryRow.objects.create(
            worksheet=worksheet,
            beneficiary=officer,
            beneficiary_type="individual",
            proposed_distribution=Decimal(str(proposed_distribution)),
        )

    def test_finalise_under_allocation_error_message(self):
        """Gate must clearly distinguish under-allocation from over-allocation."""
        worksheet = self._create_tax_planning_worksheet(distributable_income="10000")
        self._add_beneficiary_row(worksheet, proposed_distribution="8000")

        self.client.force_login(self._create_admin_user())
        response = self.client.post(
            reverse("core:tax_planning_finalise", kwargs={"pk": worksheet.financial_year.id})
        )
        self.assertEqual(response.status_code, 200)
        body = response.json()
        self.assertFalse(body.get("success"))
        joined_errors = " ".join(body.get("errors", []))
        self.assertIn("not fully allocated", joined_errors)
        self.assertIn("$2,000.00", joined_errors)
        self.assertIn("clicked Save", joined_errors)

    def test_finalise_over_allocation_error_message(self):
        """Gate must mention over-allocation explicitly when distributions exceed income."""
        worksheet = self._create_tax_planning_worksheet(distributable_income="10000")
        self._add_beneficiary_row(worksheet, proposed_distribution="12000")
        ...
        self.assertFalse(body.get("success"))
        joined_errors = " ".join(body.get("errors", []))
        self.assertIn("exceed distributable income", joined_errors)
        self.assertIn("$2,000.00", joined_errors)
        self.assertIn("clicked Save", joined_errors)

    def test_save_row_endpoint_persists_value(self):
        """Auto-save endpoint must persist proposed_distribution to the DB."""
        worksheet = self._create_tax_planning_worksheet(distributable_income="10000")
        row = self._add_beneficiary_row(worksheet, proposed_distribution="0")

        self.client.force_login(self._create_admin_user())
        response = self.client.post(
            reverse(
                "core:tax_planning_save_row",
                kwargs={"fy_id": worksheet.financial_year.id, "row_id": row.id},
            ),
            data={"proposed_distribution": "5000.00"},
        )
        self.assertNotEqual(
            response.status_code, 500,
            msg=f"save_row returned 500. Body: {response.content[:500]!r}",
        )
        self.assertEqual(response.status_code, 200)

        row.refresh_from_db()
        self.assertEqual(row.proposed_distribution, Decimal("5000.00"))
