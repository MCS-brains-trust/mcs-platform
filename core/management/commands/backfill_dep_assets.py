...
IMPORTANT: This command avoids querying Entity.objects directly because the
Entity model has an EncryptedCharField (tfn) which requires FIELD_ENCRYPTION_KEY
to be set. Instead we iterate via FinancialYear / TrialBalanceLine, which
avoids triggering the encrypted field converter.

Usage:
    python manage.py backfill_dep_assets
    python manage.py backfill_dep_assets --entity-name "Eva"
    python manage.py backfill_dep_assets --entity-id <uuid>
...
from core.models import DepreciationAsset, FinancialYear, TrialBalanceLine
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="Only process the entity with this UUID",
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        # ── Build the queryset via FinancialYear to avoid touching the
        #    encrypted Entity.tfn field (which needs FIELD_ENCRYPTION_KEY).
        fy_qs = FinancialYear.objects.all().order_by(
            "entity__entity_name", "year_label"
        ).select_related("entity")

        if entity_id:
            fy_qs = fy_qs.filter(entity_id=entity_id)
